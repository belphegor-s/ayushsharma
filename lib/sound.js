/* Interface sounds, synthesised on the fly with the Web Audio API.
   ...
   so the kit costs about a kilobyte. The one exception is Leo's bark, a real recording
   fetched only after the visitor first interacts (with the synth as a fallback if it
   ever fails to load). The AudioContext is created on the first sound after the visitor has
...
let noise = null;
let idleTimer = 0;
// Recorded sounds: path while pending, AudioBuffer once decoded, null if it failed.
const SAMPLES = { woof: '/sounds/woof.mp3' };
const samples = {};
let muted = null;
...
function loadSample(name) {
  samples[name] ??= fetch(SAMPLES[name])
    .then((res) => {
...
/* A struck wooden bar: the fundamental plus the bright 4th partial real marimba bars ring
   with, which dies away much faster than the body of the note. */
function marimba(t, freq, gain, decay) {
  tone(t, { freq, gain, attack: 0.003, decay });
  tone(t, { freq: freq * 3.99, gain: gain * 0.14, attack: 0.002, decay: decay * 0.22 });
}

/* Hovering along a row of cards plays a little tune: each one strikes a note from a major
   pentatonic scale, which has no clashing intervals, so any order sounds deliberate. */
const PENTATONIC = [523.25, 587.33, 659.25, 783.99, 880];
let lastNote = -1;
function nextNote() {
  let i = Math.floor(Math.random() * (PENTATONIC.length - 1));
  if (i >= lastNote) i++;
  lastNote = i;
  return PENTATONIC[i];
}
...
const SOUNDS = {
  ...
  // Hover sounds: one voice per kind of element, so each part of the page has its own feel.
  // Nav items: a barely-there tick.
  hover(t) {
    tone(t, { freq: vary(2600, 0.03), gain: 0.018, attack: 0.002, decay: 0.03 });
  },
  // Buttons: a short upward chirp, like the button is leaning in.
  hoverButton(t) {
    tone(t, { freq: vary(1700, 0.02), to: 2300, gain: 0.03, attack: 0.003, decay: 0.05, glide: 0.035 });
  },
  // Social link cards: a small glassy ting.
  hoverLink(t) {
    bell(t, vary(2093, 0.015), 0.02, 0.16);
  },
  // Featured project cards: a warm marimba note from the scale.
  hoverCard(t) {
    marimba(t, nextNote(), 0.05, 0.28);
  },
  // Compact project rows: the same scale an octave down, drier and quieter.
  hoverRow(t) {
    marimba(t, nextNote() / 2, 0.045, 0.14);
  },
  ...
  woof(t) {
    if (samples.woof instanceof AudioBuffer) return sample(t, samples.woof, 0.3);
...
const COOLDOWN = { hover: 60, hoverButton: 60, hoverLink: 60, hoverCard: 80, hoverRow: 60, tap: 30 };
...
export function play(name) {
  ...
  // A recording still on its way is waited for; everything else plays straight away.
  const pending = name in SAMPLES && samples[name] instanceof Promise ? samples[name] : null;
  Promise.all([pending, ctx.state === 'running' || ctx.resume()]).then(render, () => {});
}

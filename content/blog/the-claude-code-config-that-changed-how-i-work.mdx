...
The caveman plugin is a `UserPromptSubmit` hook that strips your prompt down to the essentials: no articles (a, an, the), no filler words (just, really, basically, actually), no pleasantries (sure, certainly, of course), no hedging. If you type _"Can you please help me figure out why this function is basically just returning null instead of the expected value?"_, the plugin rewrites that to _"Why function return null instead of expected value?"_ before it hits the model.
